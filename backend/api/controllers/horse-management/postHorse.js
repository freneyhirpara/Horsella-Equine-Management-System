...
const CreateHorse = require('../../../utilities/db/horse/postHorse');
...
const postHorse = async (req, resp) => {
  try {
    if (req.body === undefined || req.body === null) {
      ...
      horse.ownerId = req.userId;
      if (await checkIfUserExists(horse.ownerId)) {
        CreateHorse(horse)
          .then((values) => {
        ...
        returnError({ message: 'INV_OWNER_ID' }, resp);
      }
    }
  } catch (err) {
