...
const putbreedingCenter = require('../../../utilities/db/breeding/updateBreedingCenter');
...
const validateCredentials = ({
  ...
  if (
    ...
    throw new Error('INV_WORKING_HOURS');
  }
...
const updatebreedingCenter = async (req, resp) => {
  try {
    if (req.body === undefined || req.body === null) {
      throw new Error('DATA_ERR');
    } else {
      breedingCenter.id = parseInt(req.params.id, 10);
      if (await checkIfBreedingCenterExists(breedingCenter.id)) {
        validateCredentials(req.body);
        putbreedingCenter(breedingCenter)
          .then((values) => {
...
module.exports = updatebreedingCenter;
